...
router.delete('/:id',async(req,res)=>{
     try{
       ...
       await User.deleteOne({_id:id});
        // await Order.delete({userid:id});
        // await Orderdetail.deleteOne({id:id})
                    ...
                    for(let ordr of order){
                      totalquantity= totalquantity + (ordr.quantity);
                        ...
                        const profit = products.profit;
                          tprofit = tprofit + ((profit/100)*(ordr.totalcost));
                         ...
                         const updatequantity = await Product.updateOne({_id:ordr.productid},body);
                          ...
                          let ttotal = ordr.totalcost;

                           let tax = (products.tax);
                            additionaltax = additionaltax +  ((tax/100)*ttotal);
   ...
   let tprofit = 0
    for(let orderdetail of orderdetails){
      ...
      totalcost = totalcost + orderdetail.GrandTotal;
       ...
       tprofit = tprofit + orderdetail.TotalProfit;
 }
            let data = {
         "totaluser" :  totaluser,
